...
- [Protocole OCPP 1.6](#protocole-ocpp-16)
  ...
  - [Heartbeat et surveillance des connexions](#heartbeat-et-surveillance-des-connexions)
...
| Protocole OCPP | ocpp-rpc (OCPP 1.6-J, OCPP 2.0.1 ready) |
...
Le serveur utilise deux mécanismes complémentaires pour détecter les bornes déconnectées ou silencieuses.

#### Ping WebSocket (niveau transport)
La bibliothèque `ocpp-rpc` envoie des trames ping WebSocket à chaque borne à son intervalle par défaut (30 secondes). En l'absence de pong, la connexion est immédiatement interrompue. Ce mécanisme détecte les coupures réseau (perte de lien 4G, timeout NAT, etc.) en ~60 secondes, indépendamment de la configuration OCPP.

Le report d'activité (`deferPingsOnActivity`) est activé : si une borne envoie activement des messages OCPP (ex. `MeterValues`), le ping est reporté jusqu'à ce que la connexion soit idle, ce qui réduit le trafic inutile sur les connexions à la consommation.

#### Watchdog heartbeat (niveau applicatif)
Un watchdog côté serveur vérifie périodiquement si chaque borne connectée a bien envoyé un message `Heartbeat` récemment. Une borne n'ayant pas envoyé de heartbeat depuis `HeartbeatInterval × 2` secondes est déconnectée de force.
